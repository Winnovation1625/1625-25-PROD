...
public class AuxControllerUtil {
    ...
    public static CoralObjective getCoralObjective(List<Boolean> reefPositionButtons, List<Boolean> reefLevelButtons) {
        int reefPositionIndex = getTrueIndex(reefPositionButtons);
        int reefLevelIndex = getTrueIndex(reefLevelButtons);
        return CoralObjective.builder().position(ReefPosition.fromIndex(reefPositionIndex)).reefLevel(ReefLevel.fromLevel(reefLevelIndex)).build();
    }
